        ...
        <div class="col-lg-6 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    <form id="signupForm" action="{{ route('admin.update.assign') }}" method="POST">
                        @csrf
                        <input type="hidden" name="id" @if(!empty($assign)) value="{{$assign->id}}" @endif>

                        <div class="mb-3">
                            <label for="ageSelect" class="form-label">SubAdmin</label>
                            <select class="form-select" name="subadmin_id" id="ageSelect">
                                <option selected disabled>Select your SubAdmin</option>
                                @foreach ($subadmins as $subadmin)
                                    <option value="{{ $subadmin->id }}"
                                        @if(!empty($assign) && $assign->subadmin_id == $subadmin->id) selected @endif>
                                        {{ $subadmin->name }}
                                    </option>
                                @endforeach
                        ...
                        <div class="mb-3">
                            ...
                            @if(!empty($assign)) value="{{$assign->assign}}" @endif>
